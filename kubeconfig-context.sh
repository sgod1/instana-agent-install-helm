...
function kubeconfig_use_context() {
   ...
   if (( rc > 0 )); then echo rc=$rc, context $context not found, cluster key $cluster >&2; exit 1; fi

   context_cluster_name=`oc config get-contexts | grep "*" | tr -s ' ' | cut -d' ' -f3`; rc=$?
   if (( rc > 0 )); then echo failed to get cluster name from current context $context >&2; exit 1; fi
   ...
   if (( rc > 0 )); then echo rc=$rc, oc config view error >&2; exit 1; fi
}
